...
- `User::canAccessPanel()` always returns `true` — there is no role/permission
  ...
  authenticated users" is the correct spec wherever this plan doesn't say
  otherwise.
...
## 2. Existing Domains — Reference Specification

This section documents what already exists and works. It is not new work; it is
included so the Blueprint is a complete system map. Full namespaces are given for
reuse; field-level chains are compact since these are implemented, not planned.

### 2.1 Warehouses, Subdomains, Users, API Keys
...
### 2.2 Products, Brands, VAT Rates, Attributes
...
`vatRate(): BelongsTo` (`Modules\Settings\Models\VatRate`), `productAttributes(): HasMany`,
...
- Stock sub-page form (`ProductStockForm`, relationship `stockProduct`):
  ...
  — both must stay in sync if ever changed (they are the two independent
  implementations of the same canonical rule referenced throughout this plan).
...
Supplier model should mirror**, per the option not chosen in this plan.

### 2.4 Orders & Order Statuses
...
`completed`/`picked`/`cancelled`/`delivered`/`on_hold`, invoice/delivery address
fields, `telephone_number`, `email`, `comments`, `delivery_date` date, timestamps).
...
timestamps). Relations: `order()`, `product()`, `vatRate()`.
...
tables** — the new work in §3 completes that clone by porting the scanning logic
too.
...
`OrderStatusTransitionService::generatePicklist()`). `getPages()` registers only
`index`/`view` (no create/edit route, though the page classes exist unused on
disk). Table: `GeneratedCustomPicklistIdColumn`, `OrderGeneratedCustomOrderIdColumn`
(links to the parent Order), `ProductsCountColumn`, `ScannedProductsCountColumn`.
Row action: `ViewAction` only.

**Scanning implementation (the pattern §3 ports to Purchase Orders)** —
...
`StockProduct` — stock reduction is driven solely by the *order's* status flags
(`reduce_stock`), fully decoupled from picklist scan progress. **§3 deliberately
does something different for Purchase Orders** (scan completion *does* drive
...
### 2.6 AI Persistence & Miscellaneous Findings
...
- `Relation::enforceMorphMap()` — required by this project's custom rules for every
  ...
  this plan (not related to Purchase Orders), but worth the user's attention
  separately.
...
- **Rule (canonical, one formula, reused everywhere)**:
  ...
  calls back into `OrderStatusTransitionService` for this step, per below.
...
— add the scan handler, mirroring `ViewPicklist` exactly but against
...
`app-modules/picklists/resources/views/filament/infolists/components/products-table.blade.php`,
with two changes: dispatch event name `purchase-order-barcode-scanned` (not
`picklist-barcode-scanned`), and listen for `purchase-order-product-scan-success`
(not `picklist-product-scan-success`) for the row-highlight behavior. Reuse the
existing row id convention (`#purchase-order-product-row-{id}`, already present in
the static table markup per the exploration report) so the highlight script can
target it.
...
- Token generation in Settings already works exactly as required:
  ...
  the "generate a token in the setting section" requirement — already built.
...
- **No rename**: keep `Client`/`client_id` exactly as-is everywhere (considered
  and explicitly declined a `Client → Customer` rename).
...
exceed that product's current `StockProduct.free_on_stock_quantity` — enforced
as a **precondition check before any write**, independent of whether the chosen
`order_status`'s `reserve_stock` flag is set (a shop should not be able to
create an order with more units than are free, even against a "concept"
status). Partial fulfillment/backorder is out of scope for this pass — an
insufficient-stock item rejects the whole request.
...
2. **Stock precondition** (before any write): for each item, load its `Product`
   ...
   created.
3. **Find-or-create `Client`**: `Client::withoutGlobalScopes()->where('warehouse_id', $warehouse->id)->where('email', $data['client']['email'])->first()`;
...
surface, consistent with this project's no-Policy-classes convention (§1).
