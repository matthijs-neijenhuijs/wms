...
class ImportWithXlsxAction extends ImportAction
{
        ...
        $this->schema(fn (ImportAction $action): array => array_merge([
            FileUpload::make('file')
                ...
                ->acceptedFileTypes([
                    ...
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', // Added XLSX support
                ])
    ...
    /**
     * Override to handle XLSX files by converting them to CSV
     *
    ...
    public function getUploadedFileStream(TemporaryUploadedFile $file)
    {
        ...
        // For CSV files, use the parent method
    ...
    /**
     * Convert XLSX file to CSV stream
     *
     * @return resource
     */
    protected function convertXlsxToCsvStream(TemporaryUploadedFile $file)
    {
        ...
        // Create a temporary CSV resource in memory
        ...
        // Read the first sheet from the XLSX file
